...
class PromptToRemovePerson < WEBrick::HTTPServlet::AbstractServlet
  def do_GET(request, response)
    erb_template_string = File.read("prompt-to-remove.html.erb")
    template = ERB.new(erb_template_string)
    output   = template.result(binding)
...
class RemovePerson < WEBrick::HTTPServlet::AbstractServlet
  def do_GET(request, response)
    name = request.query["name"] # <<== replace this with getting the name from the request
    ...
    person = $database.search(name)
    if person
      $database.delete(person.name)
    end
    erb_template_string = File.read("removed.html.erb")
    template = ERB.new(erb_template_string)
    output   = template.result(binding)
